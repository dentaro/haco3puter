...
// SPIバスの設定
      ...
      cfg.freq_write = 40000000;//20000000;//40000000;    // 送信時のSPIクロック (最大80MHz, 80MHzを整数で割った値に丸められます)
      cfg.freq_read  = 16000000;    // 受信時のSPIクロック
      cfg.spi_3wire  = true;        // 受信をMOSIピンで行う場合はtrueを設定
      cfg.use_lock   = true;        // トランザクションロックを使用する場合はtrueを設定
      cfg.dma_channel = 1;          // Set the DMA channel (1 or 2. 0=disable)   使用するDMAチャンネルを設定 (0=DMA不使用)
      cfg.pin_sclk = 14;//18            // SPIのSCLKピン番号を設定
      cfg.pin_mosi = 13;//23            // SPIのMOSIピン番号を設定
      cfg.pin_miso = 12;//19            // SPIのMISOピン番号を設定 (-1 = disable)
      cfg.pin_dc   = 4;//27            // SPIのD/Cピン番号を設定  (-1 = disable)
...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.pin_cs           =    15;//14  // CSが接続されているピン番号   (-1 = disable)
      cfg.pin_rst          =     2;//33  // RSTが接続されているピン番号  (-1 = disable)
...
// SPI接続の場合
      ...
      cfg.pin_cs   = 21;//4;//5;     //   CSが接続されているピン番号
