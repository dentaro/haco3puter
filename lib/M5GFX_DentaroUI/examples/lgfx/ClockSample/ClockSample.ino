...
static uint64_t c_count = random(oneday);    // 現在時刻 (ミリ秒カウンタ)
...
void setup(void)
{
  ...
  //lcd.init();
  ui.begin( lcd, 3, 24, 3, false );//lcd, 色深度,回転方向,タッチキャリブレーション
...
void loop(void)
{
  ...
  if (p_milli < milli) c_count +=        (milli - p_milli);
  else                 c_count += 1000 + (milli - p_milli);
  p_milli = milli;

  int32_t tmp = (c_count % 1000) >> 3;
  canvas.setPaletteColor(8, 255 - (tmp>>1), 255 - (tmp>>1), 200 - tmp); // 秒針の描画色を変化させる
//c_count += 60000;
  if ( c_count > oneday ) { c_count -= oneday; }
  drawClock(c_count);
}
